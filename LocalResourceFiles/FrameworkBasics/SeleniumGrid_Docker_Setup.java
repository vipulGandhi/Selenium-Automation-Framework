	...
	// Setup docker engine
		...
		// Create container(s) on the image[docker run : pull the image(if not available) and create the container]
			...
			// Create Chrome node container
				// Terminal
					// docker run -d --name selenium-node-chrome-01  --link selenium-hub:hub -P selenium/node-chrome-debug
			// Create Firefox node container
				// Terminal
					// docker run -d --name selenium-node-firefox-01  --link selenium-hub:hub -P selenium/node-firefox-debug
			// docker run command
